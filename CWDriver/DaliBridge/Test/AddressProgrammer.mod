      ...
      FOR i := 0 TO 1<<24-1 DO
         ...
         LOOP
            ...
            | DaliBridge.getFALSE : // not found, failure
               Host^.Log^.LogSC( log.lcError, 0, L"", L"Not found, random: ", Random );
               Host^.Log^.LogSC( log.lcError, 0, L"", L"          address: ", address.C24 );
               EXIT;
            | DaliBridge.getFOUND : // ok

               IF Random = 1 * 256 + 2 * 256 * 256 + 3 * 256 * 256 * 256 THEN
                  IF address.L8 <> 1 THEN // failure
                     Host^.Log^.LogSC( log.lcError, 0, L"", L"Failure in L byte, expected 1, found: ", CARDINAL( address.L8 ));
                  END;
                  IF address.M8 <> 2 THEN // failure
                     Host^.Log^.LogSC( log.lcError, 0, L"", L"Failure in M byte, expected 1, found: ", CARDINAL( address.M8 ));
                  END;
                  IF address.H8 <> 3 THEN // failure
                     Host^.Log^.LogSC( log.lcError, 0, L"", L"Failure in H byte, expected 1, found: ", CARDINAL( address.H8 ));
                  END;
               END;
