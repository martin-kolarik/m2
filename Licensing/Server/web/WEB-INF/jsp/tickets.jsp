    ...
    <body>
        <div id="holder2">
            ...
            <div id="content2">
                ...
                <div class="fl links">
                    <a class="act" href="<c:url value="/tickets.do"/>"><spring:message code="public.tickets.ticketsLink"/></a> |
                    <a href="<c:url value="/bills.do"/>" ><spring:message code="public.tickets.billsLink"/></a>
                    <c:if test="${customer.parentCustomer==null}">
                        | <a href="<c:url value="/customers.do"/>" ><spring:message code="public.tickets.childsLink"/></a>
                    </c:if>
                </div>
                
                <div class="fr user">
                    <spring:message code="public.tickets.loginUserPhoneNumber"/>
                    <strong><c:out value="${customer.phoneNumber}"/></strong> |
                    <a href="<c:url value="/logout.do"/>"><spring:message code="logout"/></a>
                </div>
                
                <form:form id="composite" modelAttribute="ticketsData">
                
                    <div id="filter">
                        <div class="fr">
                            <label for="month"><spring:message code="public.tickets.month"/></label>
                            <form:select path="monthString" items="${monthList}"/>
                            <form:errors cssClass="errors" path="monthString"/>
                            
                            <input type="submit" name="do" value='<spring:message code="public.tickets.filter"/>' class="button"/>
                        </div>
                        <input type="submit" name="do" value='<spring:message code="public.tickets.printSelected"/>' class="button120"/>                        
                    </div>
                    
                    <c:choose>
                        <c:when test="${ticketsData.result.hasData}">
                            ...
                            <table id="list">
                                        ...
                                        <c:forEach var="orderKey" items="${orderKeys}">
                                            <c:choose>
                                                <c:when test="${!pager.sortable[orderKey]}">
                                                    <th>
                                                        <spring:message code="public.tickets.table.${orderKey}"/>
                                                    </th>
                                                </c:when>
                                ...
                                <c:forEach var="ticket" items="${ticketsData.result.data}" varStatus="status">
                                    <c:choose>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${ticket.billId==null}">
                                                    <form:checkbox path="selectedTickets" value="${ticket.id}"/>
                                                </c:when>
                                                <c:otherwise>
                                                    &nbsp;
                                                </c:otherwise>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${ticket.billId==null}">
                                                    <a href="<c:url value="/billTicket.do?id="/><c:out value="${ticket.id}"/>" class="print"><spring:message code="public.tickets.bill"/></a>
                                                </c:when>
                                                <c:otherwise>
                                                    &nbsp;
                                                </c:otherwise>
                        ...
                        <c:otherwise>
                            <p>
                                <spring:message code="public.tickets.nodata"/>
                            </p>
                        </c:otherwise>
                    </c:choose>
