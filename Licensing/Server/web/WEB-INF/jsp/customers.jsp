    ...
    <body>
        <div id="holder2">
            ...
            <div id="content2">
                ...
                <div class="fl links">
                    <a href="<c:url value="/tickets.do"/>"><spring:message code="public.customers.ticketsLink"/></a> |
                    <a href="<c:url value="/bills.do"/>" ><spring:message code="public.customers.billsLink"/></a>
                    <c:if test="${customer.parentCustomer==null}">
                        | <a class="act" href="<c:url value="/customers.do"/>" ><spring:message code="public.customers.childsLink"/></a>
                    </c:if>
                </div>
                
                <div class="fr user">
                    <spring:message code="public.customers.loginUserPhoneNumber"/>
                    <strong><c:out value="${customer.phoneNumber}"/></strong> |
                    <a href="<c:url value="/logout.do"/>"><spring:message code="logout"/></a>
                </div>
                
                <form:form modelAttribute="customersData" id="filter">
                    <div class="fr">
                        <label for="phoneNumber" class="label"><spring:message code="public.customers.phoneNumber"/></label>
                        <form:input path="phoneNumber"/>
                        <label for="hashCode" class="label"><spring:message code="public.customers.hash"/></label>
                        <form:input path="hashCode"/>                        
                        <input name="do" type="submit" value='<spring:message code="public.customers.addPhoneNumber"/>' class="rbutton"/>
                    </div>                    
                </form:form>
                
                <c:choose>
                    <c:when test="${customersData.result.hasData}">
                        ...
                        <table id="list">
                                    ...
                                    <c:forEach var="orderKey" items="${orderKeys}">
                                        <c:choose>
                                            <c:when test="${!pager.sortable[orderKey]}">
                                                <th>
                                                    <spring:message code="public.customers.table.${orderKey}"/>
                                                </th>
                                            </c:when>
                            ...
                            <c:forEach var="customer" items="${customersData.result.data}" varStatus="status">
                                <c:choose>
                                    ...
                                    <td><fmt:formatDate value="${customer.created}" pattern="d. M. yyyy H:mm"/></td>
                                    <td>&nbsp;</td>                                
                                </tr>
                            </c:forEach>
                        </table>
                    ...
                    <c:otherwise>
                        <p>
                            <spring:message code="public.customers.nodata"/>
                        </p>
                    </c:otherwise>
                </c:choose>
