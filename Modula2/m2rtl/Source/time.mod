...
PROCEDURE SystemTimeToJD( CONST SystemTime : windows.SYSTEMTIME ) : TJD;
BEGIN
  RETURN JD( INTEGER( SystemTime.wYear ),
             INTEGER( SystemTime.wMonth ),
             INTEGER( SystemTime.wDay ),
             HMS2fd( CARDINAL( SystemTime.wHour ), CARDINAL( SystemTime.wMinute ), CARDINAL( SystemTime.wSecond ), CARDINAL( SystemTime.wMilliseconds )));
END SystemTimeToJD;

(*---------------------------------------------------------------------------*)

PROCEDURE JDToSystemTime( JD : TJD; OUT SystemTime : windows.SYSTEMTIME );
VAR
  C1, C2, C3, C4 : CARDINAL;
  FD             : CARDINAL;
  I1, I2, I3     : INTEGER;
BEGIN
  iJD( JD, OUT I1, OUT I2, OUT I3, OUT FD );
  WITH SystemTime DO
    wYear := WORD( I1 );
    wMonth := WORD( I2 ); 
    wDay := WORD( I3 );
    wDayOfWeek := WORD( CARDINAL( ToSJD( JD ) + 1.5 ) MOD 7 ); // ( JD + 0.5 ) MOD 7 gives 0 = Monday

    fd2HMS( FD, OUT C1, OUT C2, OUT C3, OUT C4 );
    wHour := WORD( C1 );
    wMinute := WORD( C2 );
    wSecond := WORD( C3 );
...
PROCEDURE DateTimeToSystemTime( CONST DateTime : TDateTime; OUT SystemTime : windows.SYSTEMTIME );
BEGIN
...
PROCEDURE SystemTimeToDateTime( CONST SystemTime : windows.SYSTEMTIME; OUT DateTime : TDateTime );
BEGIN
	InitDateTime( OUT DateTime );
...
VAR
  ...
  FFlag : BOOLEAN;
BEGIN
  DateTimeToSystemTime( DateTime, OUT st );
