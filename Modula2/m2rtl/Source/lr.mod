...
CONST
   fullBits = BITS64( 0FFFFFFFFFFFFFFFFH );
   signBit = 63;
   signMask = BITS64( 08000000000000000H );
   signBitHi = 31;
   signMaskHi = BITS32( 080000000H );
   expMask = BITS64( 07FF0000000000000H );
   expMaskHi = BITS32( 07FF00000H );
   fracMask = BITS64( 0000FFFFFFFFFFFFFH );
...
PROCEDURE ExponentBits( CONST lr : LONGREAL ) : BITS64;
BEGIN
   RETURN PLR2B( ADR( lr ))^.bits * expMask;
END ExponentBits;

//--------------------------------------------------------------------------------

PROCEDURE FractionBits( CONST lr : LONGREAL ) : BITS64;
BEGIN
   RETURN PLR2B( ADR( lr ))^.bits * fracMask;
END FractionBits;
...
PROCEDURE TwoExponent( CONST R : LONGREAL ) : INTEGER;
BEGIN
   RETURN CARD32( PLR2B( ADR( R ))^.bitshi * expMaskHi ) >> expShiftHi - expBias;
END TwoExponent;
  
//--------------------------------------------------------------------------------

PROCEDURE TenExponent( CONST R : LONGREAL ) : INTEGER;
CONST
   log2 = 0.301029995663981;
VAR
   Exp10 : INTEGER;
BEGIN
   Exp10 := INTEGER( LONGREAL( TwoExponent( R )) * log2 );
...
PROCEDURE Pow10( exp : INTEGER ) : LONGREAL;
BEGIN
   IF ( exp > 308 ) OR ( exp < -323 ) THEN
      RETURN 0.0;
   ELSE
      RETURN pow10[exp];
   END;
...
PROCEDURE IsNaN( CONST lr : LONGREAL ) : BOOLEAN;
BEGIN
   RETURN ( ExponentBits( lr ) = expMask ) AND ( FractionBits( lr ) <> BITS32{} );
END IsNaN;
...
PROCEDURE Equals( CONST lr1, lr2 : LONGREAL; ulps : CARDINAL ) : BOOLEAN;
VAR
   lr2b1, lr2b2 : LR2B;
BEGIN
   IF IsNaN( lr1 ) OR IsNaN( lr2 ) THEN
      RETURN FALSE;
   END;

   // bias first input
   lr2b1.lr := lr1;
   IF signMaskHi * lr2b1.bitshi = BITS32{} THEN // positive number, bias it
      INCL( lr2b1.bitshi, signBitHi );
   ELSE // negative number, convert it
      lr2b1.bits := lr2b1.bits / fullBits;
      INC( lr2b1.number, 1 );
   END;

   // bias second input
   lr2b2.lr := lr2;
   IF signMaskHi * lr2b2.bitshi = BITS32{} THEN // positive number, bias it
      INCL( lr2b2.bitshi, signBitHi );
   ELSE // negative number, convert it
      lr2b2.bits := lr2b2.bits / fullBits;
      INC( lr2b2.number, 1 );
   END;

   // compare biased values
   IF lr2b1.number >= lr2b2.number THEN
      RETURN lr2b1.number - lr2b2.number <= CARD64( ulps );
   ELSE
      RETURN lr2b2.number - lr2b1.number <= CARD64( ulps );
   END;
END Equals;
