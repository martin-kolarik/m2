...
(*---------------------------------------------------------------------------*)
   ...
   BEGIN
      ...
      ia.FromOA( L"192.169.1.2", 0, NIL );
      AL.AddRule( AccessList.actAllow, ia, 14 );

      AL.AddRuleOA( AccessList.actDeny, L"192.168.0.63/26" );
      
      ia.FromOA( L"192.168.0.100", 0, NIL ); // allowed
      IF NOT AL.AllowedForAddress( ia ) THEN
         Failure := TRUE;
      END;

      ia.FromOA( L"192.168.1.128", 0, NIL ); // allowed
      IF NOT AL.AllowedForAddress( ia ) THEN
         Failure := TRUE;
      END;

      ia.FromOA( L"192.169.1.128", 0, NIL ); // allowed
      IF NOT AL.AllowedForAddress( ia ) THEN
         Failure := TRUE;
      END;

      ia.FromOA( L"192.172.1.128", 0, NIL ); // disallowed
      IF AL.AllowedForAddress( ia ) THEN
         Failure := TRUE;
      END;

      ia.FromOA( L"192.168.0.1", 0, NIL ); // disallowed
      IF AL.AllowedForAddress( ia ) THEN
         Failure := TRUE;
      END;
