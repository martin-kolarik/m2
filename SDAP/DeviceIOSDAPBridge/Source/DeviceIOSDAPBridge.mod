         ...
         IF Result = Sync.arTimeout THEN // read data from device and send it to SDAP
            ...
            WHILE _Data.MoveNext() DO
               ...
               IF NOT _Logger.Filtered( log.ldDebug, 0, LOG_NAME ) THEN
                  _Logger.LogSS( log.ldDebug, 0, LOG_NAME, L"Querying: ", OA( item^.SDAPName.Length-1, item^.SDAPName.rawData ));
               END;
               ...
               IF ( Result <> Sync.arCompleted ) AND ( Result <> Sync.arPending ) THEN
                  ...
                  _Logger.LogSR( log.ldTrace, 0, L"IO", OA( s.Length-1, s.rawData ), Result );
                  CONTINUE;
               END;
               ...
               IF NOT _Logger.Filtered( log.ldDebug, 0, LOG_NAME ) THEN
                  _Logger.LogSS( log.ldDebug, 0, LOG_NAME, L"Got value: ", OA( valueString.Length-1,  valueString.rawData ));
               END;
               ...
               IF Result <> Sync.arCompleted THEN
                  ...
                  s.Append( item^.SDAPName );
                  _Logger.LogSR( log.ldTrace, 0, LOG_NAME, OA( s.Length-1, s.rawData ), Result );
               END;
            END; // WHILE
         ...
         ELSE // write data to device
            LOOP
               ...
               WHILE _Data.MoveNext() DO
                  ...
                  IF NOT _Logger.Filtered( log.ldDebug, 0, LOG_NAME ) THEN
                     _Logger.LogSSSS( log.ldDebug, 0, LOG_NAME, L"Writing: ", OA( item^.SDAPName.Length-1, item^.SDAPName.rawData ), L"", OA( valueString.Length-1, valueString.rawData ));
                  END;
